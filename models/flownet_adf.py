...
class _FlowNetADF(nn.Module):
    ...
    def forward(self, *inputs):
        ...
        concat5 = concatenate_as(
            (conv5_1, deconv5, upsampled_flow6_to_5), conv5_1, dim=1)
        ...
        concat4 = concatenate_as(
            (conv4_1, deconv4, upsampled_flow5_to_4), conv4_1, dim=1)
        ...
        concat3 = concatenate_as(
            (conv3_1, deconv3, upsampled_flow4_to_3), conv3_1, dim=1)
        ...
        concat2 = concatenate_as(
            (conv2, deconv2, upsampled_flow3_to_2), conv2, dim=1)
...
class FlowNetADF(nn.Module):
    def __init__(self, args, noise_variance=1e-3, min_variance=1e-3, div_flow=0.05, log_variance=False):
        ...
        self._flownetadf = _FlowNetADF(
            args, keep_variance_fn=self._keep_variance_fn)
    ...
    def forward(self, input_dict):
        ...
        output_dict = {}
        if self.training:
            flow2, flow3, flow4, flow5, flow6 = self._flownetadf(
                inputs_mean, inputs_variance)
            ...
            flow2_mean, flow2_variance = self._flownetadf(
                inputs_mean, inputs_variance)

            flow1_mean = (1.0 / self._div_flow) * \
                upsample2d_as(flow2_mean, im1, mode="bilinear")
            flow1_variance = (1.0 / self._div_flow)**2 * \
                upsample2d_as(flow2_variance, im1, mode="bilinear")
